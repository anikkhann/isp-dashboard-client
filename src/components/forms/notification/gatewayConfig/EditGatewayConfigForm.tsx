...
interface PropData {
  item: GatewayConfigData;
}

const EditGatewayConfigForm = ({ item }: PropData) => {
  ...
  useEffect(() => {
    if (item) {
      form.setFieldsValue({
        ...
        smsGatewayId: item.smsGatewayId,
        credentials: item.credentials
      });
      setSelectedClient(item.clientId);
      setSelectedSmsGateway(item.smsGatewayId);
  ...
  const onSubmit = (data: FormData) => {
    ...
    try {
      axios
        .put("/api/sms-gateway-config/update", formData)
        .then(res => {
          const { data } = res;
...
export default EditGatewayConfigForm;
