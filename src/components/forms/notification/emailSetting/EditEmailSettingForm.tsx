...
interface PropData {
  item: EmailSettingData;
}

const EditEmailSettingForm = ({ item }: PropData) => {
  ...
  const [selectedAuthType, setSelectedAuthType] = useState<any>(null);
  ...
  useEffect(() => {
    if (item) {
      form.setFieldsValue({
        smtpAccountName: item.smtpAccountName,
      ...
      setSelectedAuthType(item.authType);
  ...
  const onSubmit = (data: FormData) => {
    ...
    try {
      axios
        .put("/api/email-settings/update", formData)
        .then(res => {
          const { data } = res;
...
export default EditEmailSettingForm;
