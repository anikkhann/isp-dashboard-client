...
type PropData = {
  item: NoticeBoardData;
};

const EditNoticeForm = ({ item }: PropData) => {
  ...
  useEffect(() => {
    if (item) {
      setSelectedNoticeType(item.noticeType);
      ...
      form.setFieldsValue({
        ...
        zoneManagerId: item.zoneManagerId,
        subZoneManagerId: item.subZoneManagerId,
  ...
  const onSubmit = () => {
    ...
    try {
      axios
        .put("/api/notice-board/update", formData)
        .then(res => {
          const { data } = res;
