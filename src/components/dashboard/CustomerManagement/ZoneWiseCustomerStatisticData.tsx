...
const ZoneWiseCustomerStatisticData = () => {
  ...
  const fetchData = async () => {
    ...
    const { data } = await axios.get(
      `/api/dashboard/get-total-customer-zone-wise`,
      {
        headers: {
  ...
  const { isLoading, isError, error, isFetching } = useQuery<boolean, any>({
    queryKey: ["dashboard-get-total-customer-zone-wise-cm"],
    queryFn: async () => {
      ...
      <>
        <AppRowContainer>
          <Col span={24} key="data-f">
            ...
            <TableCard
              title="Customer Statistic (Zone Wise)"
              hasLink={false}
              addLink=""
              permission=""
              style={{
...
export default ZoneWiseCustomerStatisticData;
