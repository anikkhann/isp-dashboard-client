...
const GatewayConfigList: React.FC = () => {
  ...
  const fetchData = async (
    ...
    const { data } = await axios.post(
      "/api/sms-gateway-config/get-list",
      body,
      {
        headers: {
  ...
  const { isLoading, isError, error, isFetching } = useQuery<boolean, any>({
    queryKey: ["sms-gateway-config-list", page, limit, order, sort],
    queryFn: async () => {
  ...
  const columns: ColumnsType<DataType> = [
    ...
    },
    {
      ...
      render: (smsGateway: any) => {
        return (
          <>
            <Space>{smsGateway ? smsGateway.name : "-"}</Space>
          </>
        );
      },
      width: "20%",
...
export default GatewayConfigList;
