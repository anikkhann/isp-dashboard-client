...
const DetailsUnusedVoucher = ({ id }: any) => {
  ...
  const fetchData = async () => {
    ...
    const response = await axios.get(`/api-hotspot/voucher/get-by-id/${id}`);
    return response;
  };

  const { isLoading, isError, error, isFetching } = useQuery<boolean, any>({
    queryKey: ["voucher-list", id],
    queryFn: async () => {
      ...
      <AppRowContainer>
        <Breadcrumb
          ...
          items={[
            ...
            {
              title: (
                <Link href="/admin/hotspot/unused-voucher">
                  Unused Voucher List
                </Link>
              )
            },
...
export default DetailsUnusedVoucher;
