...
import EditNasDeviceForm from "@Components/forms/nas-device/EditNasDeviceForm";
import { NasDeviceData } from "@/interfaces/NasDeviceData";

const EditNas = ({ id }: any) => {
  const [item, SetItem] = useState<NasDeviceData | null>(null);
  const fetchData = async () => {
    const token = Cookies.get("token");
    axios.defaults.headers.common["Authorization"] = `Bearer ${token}`;

    const response = await axios.get(`/api-hotspot/nas-device/get-by-id/${id}`);
    return response;
  };

  const { isLoading, isError, error, isFetching } = useQuery<boolean, any>({
    queryKey: ["nas-device-list", id],
    queryFn: async () => {
    ...
    <>
      <AppRowContainer>
        ...
        >
          ...
          {!isLoading && item && <EditNasDeviceForm item={item} />}
        </Card>
      </AppRowContainer>
    </>
  );
