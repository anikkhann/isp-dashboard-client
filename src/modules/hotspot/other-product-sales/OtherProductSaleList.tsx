...
const OtherProductSaleList: React.FC = () => {
  ...
  const fetchData = async (
    ...
    const { data } = await axios.post(
      `/api-hotspot/other-product-sales/get-list`,
      body,
      {
        headers: {
    ...
    try {
      ...
      if (result.isConfirmed) {
        ...
        const { data } = await axios.put(
          `/api-hotspot/other-product-sales/cancel`,
          body
        );
        if (data.status === 200) {
    ...
    try {
      ...
      if (result.isConfirmed) {
        const { data } = await axios.get(
          `/api-hotspot/other-product-sales/approve/${id}`
        );
        if (data.status === 200) {
          MySwal.fire("Success!", data.message, "success").then(() => {
            router.reload();
        ...
        return (
          <div className="flex flex-row">
            <Space size="middle" align="center">
              {ability.can("OtherProductSale.approve", "") ? (
              ...
              {ability.can("OtherProductSale.cancel", "") ? (
              ...
              {ability.can("OtherProductSale.update", "") ? (
    ...
    <>
      <AppRowContainer>
        <Col span={24} key="data-f">
          ...
          <TableCard
            title="Other Product Sale List"
            hasLink={true}
            addLink="/admin/hotspot/other-product-sale/create"
            permission="OtherProductSale.create"
            style={{
              borderRadius: "10px",
              padding: "10px",
              width: "100%",
...
export default OtherProductSaleList;
