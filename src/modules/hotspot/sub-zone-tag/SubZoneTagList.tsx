...
const SubZoneTagList: React.FC = () => {
  const [data, setData] = useState<SubZoneTagData[]>([]);
  ...
  const columns: ColumnsType<SubZoneTagData> = [
    {
      title: "Serial",
      dataIndex: "id",
  ...
  const handleTableChange = (
    pagination: TablePaginationConfig,
    filters: Record<string, FilterValue | null>,
    sorter: SorterResult<SubZoneTagData> | SorterResult<SubZoneTagData>[]
  ) => {
    SetPage(pagination.current as number);
    SetLimit(pagination.pageSize as number);

    if (sorter && (sorter as SorterResult<SubZoneTagData>).order) {
      // // console.log((sorter as SorterResult<SubZoneTagData>).order)

      SetOrder(
        (sorter as SorterResult<SubZoneTagData>).order === "ascend"
          ? "asc"
          : "desc"
      );
    }
    if (sorter && (sorter as SorterResult<SubZoneTagData>).field) {
      // // console.log((sorter as SorterResult<SubZoneTagData>).field)

      SetSort((sorter as SorterResult<SubZoneTagData>).field as string);
    }
  };
