...
const RetailerRevenueDisbursementList: React.FC = () => {
    ...
    try {
      ...
      if (result.isConfirmed) {
        ...
        const { data } = await axios.put(
          `/api/retailer-revenue-disbursement/update`,
          body
        );
        if (data.status === 200) {
  ...
  const fetchData = async (
    ...
    const { data } = await axios.post(
      "/api/retailer-revenue-disbursement/get-list",
      body,
      {
        headers: {
  ...
  const { isLoading, isError, error, isFetching } = useQuery<boolean, any>({
    queryKey: ["retailer-revenue-disbursement-list", page, limit, order, sort],
    queryFn: async () => {
    ...
    <>
      <AppRowContainer>
        <Col span={24} key="data-f">
          ...
          <TableCard
            title="Retailers Revenue Disbursement List"
            hasLink={true}
            addLink="/admin/accounting/retailer-revenue-disbursement/create"
            permission="retailerRevenueDisbursement.create"
            style={{
              borderRadius: "10px",
              padding: "10px",
              width: "100%",
...
export default RetailerRevenueDisbursementList;
