...
const ExpireDetails = () => {
  ...
  const fetchData = async (
    ...
    const { data } = await axios.get(
      `/api/dashboard/get-expired-customer-list`,
  ...
  const { isLoading, isError, error, isFetching } = useQuery<boolean, any>({
    queryKey: ["expire-details", page, limit, order, sort],
    queryFn: async () => {
  ...
  const handleDownload = async () => {
    ...
    await axios
      .get(`/api/dashboard/get-expired-customer-list`, {
        headers: {
          "Content-Type": "application/json"
        }
      })
            ...
            <Row justify={"end"}>
              <Col span={3}>
                ...
                <CSVLink
                  ...
                  ref={downloadRef}
                  target="_blank"
                  filename={`expire-details-list-${dayjs().format(
                    "YYYY-MM-DD"
                  )}.csv`}
                  style={{
                    display: "none"
